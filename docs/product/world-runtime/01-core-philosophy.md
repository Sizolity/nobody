...
`nobody` should evolve from a narrow narrative harness into a reusable **fictional world runtime**.

The project should provide the lower-level engine for systems such as:

- novel generation and continuation;
- RPG and open-world roleplay;
- persistent character simulation;
- self-driven fictional worlds;
- externally driven scenario execution;
- random incident generation;
- rehearsal, deduction, and counterfactual exploration;
- structured state optimization for long-running stories.

The recommended center is `World`, with a simulation-style runtime. `World` owns durable state. Runtime systems propose, validate, apply, remember, and render changes.

## Core Thesis

`World` is the stable root abstraction.

`nobody` should not make `Story`, `Game`, or `Character` the root of the system:
...
The core engine should preserve this separation:
...
This lets the same lower layer support fiction writing, RPG roleplay, world simulation, debug replay, and product-specific interfaces.

## Design Boundaries

The world runtime should treat generated prose, UI state, and product-specific game loops as projections of the underlying world, not as the world itself.

Core boundaries:

- Products use the runtime; they do not define the core data model.
- Views render world state; they do not become the source of truth.
- Runtime changes world state only through events.
- Rules validate and resolve changes before they apply.
- Memory records can represent subjective belief, not only objective facts.
- Characters should act on their own visible context, not omniscient world truth.

## Important Non-Goals

The core runtime should avoid these early traps:
