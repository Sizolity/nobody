...
func TestE2E_LLMDirector_DefaultPrompt_WithEffects(t *testing.T) {
	if os.Getenv("NOBODY_E2E_DEEPSEEK") == "" {
		t.Skip("set NOBODY_E2E_DEEPSEEK=1 and DEEPSEEK_API_KEY to run")
	}
	apiKey := os.Getenv("DEEPSEEK_API_KEY")
	...
	gen := NewDeepSeekGenerator(DeepSeekGeneratorConfig{APIKey: apiKey})

	d := NewLLMDirector("e2e_effects", LLMDirectorConfig{
		Generator: gen,
	})
	...
	world := model.World{
		ID:          "e2e_world",
		Name:        "The Kingdom of Shadows",
		Description: "A medieval fantasy kingdom shrouded in mystery.",
		Entities: map[model.EntityID]model.Entity{
			"char_elara": {
				ID: "char_elara", Type: "character", Name: "Elara the Scholar",
				Description: "A curious scholar investigating ancient ruins.",
				State: map[string]model.Value{
					"mood": {Kind: model.ValueKindString, Raw: "determined"},
				},
			},
			"tavern": {ID: "tavern", Type: "location", Name: "The Dusty Lantern"},
		},
		Facts: []model.Fact{{
			ID: "fact_gate_status", SubjectID: "city_gate",
			Predicate: "status",
	...
	for i, e := range events {
		...
		t.Logf("event[%d]: id=%s type=%s effects=%d desc=%q",
			i, e.ID, e.Type, len(e.Effects), e.Description)
		for j, eff := range e.Effects {
			t.Logf("  effect[%d]: kind=%s target=%s payload_keys=%d",
				j, eff.Kind, eff.TargetID, len(eff.Payload))
		}
	}
}
