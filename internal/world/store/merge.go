...
func cloneWorld(w model.World) model.World {
	out := w

	if w.Entities != nil {
		out.Entities = make(map[model.EntityID]model.Entity, len(w.Entities))
		for k, v := range w.Entities {
			out.Entities[k] = v
		}
	}
	out.Facts = cloneSlice(w.Facts)
	out.Relations = cloneSlice(w.Relations)
	out.Memory = cloneSlice(w.Memory)
	out.EventLog = cloneSlice(w.EventLog)
	out.Rules = cloneSlice(w.Rules)
	out.Threads = cloneSlice(w.Threads)
	out.EventQueue = cloneSlice(w.EventQueue)
	return out
}
