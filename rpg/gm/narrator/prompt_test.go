...
func TestSystemPrompt_IncludesNPCMemorySection(t *testing.T) {
	...
	// Section must appear between ## Characters and ## Locations
	idxChars := strings.Index(prompt, "## Characters")
	idxNPC := strings.Index(prompt, "## NPC 记忆")
	idxLocs := strings.Index(prompt, "## Locations")
	if !(idxChars < idxNPC && idxNPC < idxLocs) {
		t.Errorf("section placement wrong: chars=%d npc=%d locs=%d", idxChars, idxNPC, idxLocs)
	}
}
